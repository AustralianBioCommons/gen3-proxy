...
export function validateConfig(config: AppConfig): void {
  ...
  for (const stage of config.stages) {
    ...
    if (!stage.networkLookup?.proxiedSubnetIdsParameterName) {
      throw new Error(
        `stage ${stage.id}: networkLookup.proxiedSubnetIdsParameterName is required`
      );
    }
    if (!stage.networkLookup?.publicSubnetIdsParameterName) {
      throw new Error(
        `stage ${stage.id}: networkLookup.publicSubnetIdsParameterName is required`
      );
    }
    ...
    if (!stage.proxy) {
      throw new Error(`stage ${stage.id}: proxy is required`);
    }
  }
}
